...
class GposIntrospector:
    ...
    def iter_chain_context_pos_subtables(
        self, formats: set[int] | None = None
    ) -> Iterator[tuple[int, Any]]:
        for lookup_index, subtable in self.iter_lookup_subtables(
            otTables.ChainContextPos.LookupType
        ):
            if formats is None or subtable.Format in formats:
                yield lookup_index, subtable

    def chain_context_pos_subtable_counts_by_lookup(
        self, formats: set[int] | None = None
    ) -> dict[int, int]:
        counts: dict[int, int] = {}
        for lookup_index, _subtable in self.iter_chain_context_pos_subtables(formats):
