...
def check_sbmufl_contextual_subtable_count(
    ...
    gpos = GposIntrospector(ttFont)
    counts_by_lookup = gpos.chain_context_pos_subtable_counts_by_lookup()
    ...
    if lookup_count != SBMUFL_CONTEXTUAL_LOOKUP_COUNT:
        ...
        yield fail(
            ...
            "Generated contextual positioning uses "
            f"{lookup_count} matching contextual GPOS lookups; expected "
    ...
    if oversized_counts:
        ...
        yield fail(
            ...
            "Generated contextual positioning exceeds the contextual GPOS "
...
def check_sbmufl_ison_contextual_positioning(ttFont: Any, config: Any) -> CheckIterator:
    ...
    inconsistent_positions = [
        f"{glyph_name}: {format_mark_class(mark_class)}, X={x}, Y={y} "
...
@check(
    ...
    rationale="""
        Koronis is a mark that may stack above gorgon-family and fthora/chroa
        marks. Generated
