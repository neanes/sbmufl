...
class _SbmuflMetadata(object):
        ...
        for char in self.font:
            ...
            for anchor in char.anchorPoints:
                ...
                if anchor_name in SbmuflFont.valid_anchor_names:
                    x, y = (float((decimal.Decimal.from_float(value) / decimal.Decimal(self.font.em)))
    ...
    def bounding_boxes(self):
        ...
        for char in self.font:
            char_name = self.font.canonical_glyphname(char)
            xmin, ymin, xmax, ymax = (
                (float(decimal.Decimal.from_float(value) / decimal.Decimal(self.font.em))) for value in char.boundingBox())
